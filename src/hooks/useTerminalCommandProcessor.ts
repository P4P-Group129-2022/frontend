...
export const useTerminalCommandProcessor = () => {
    ...
    try {
      ...
      switch (response.data.status) {
        ...
        case "unmodified":
          return {
            input,
            output: [{ value: onBranchMain }, { value: nothingToCommit }],
          };
        default:
          return {
            input,
    ...
    try {
      if (fileNames.length === 0) {
        ...
        const responses = await Promise.all(
          fileNames.map(async (fileName) => stageFileInRepo(username, fileName))
        );
          ...
          output:
            failedRequests.length === 0
              ...
              : [
                  ...
                  {
                    value: `Failed files: ${failedRequests
                      .map((response) => response.data?.fileName)
                      .join(", ")}`,
                  },
                ],
        };
    ...
    try {
      ...
      if (commitArgs === "-m" || commitArgs === "-am") {
        ...
        checkAndAdvanceScenarioSegment(
          isAddCommit ? TaskType.ADDCOMMIT : TaskType.COMMIT
        );
  ...
  async function processGitPush(
    args: string[],
    accessToken?: string
  ): Promise<ConsolePrint> {
    const input = `git push ${args.join(" ")}`;
    const [remote, branch] = args;
    const pushHint =
      "hint: Use 'git push origin <branch>' to push a branch to a remote repository, such as 'main'.";
    ...
    if (remote.startsWith("-") || branch.startsWith("--")) {
      return {
        ...
        output: [
          ...
          {
            value:
              "Currently, the app does not support git push arguments, such as -u for setting the upstream.",
          },
          { value: pushHint },
        ],
      };
    ...
    if (!accessToken) {
      return {
        ...
        output: [
          ...
          {
            value:
              "Please go to the main website to re-login and resume scenarios.",
          },
          {
            value:
              "Or contact the administrator for assistance, through the email here: hpar461@aucklanduni.ac.nz",
          },
        ],
      };
    }
    ...
    try {
      ...
      return {
        ...
        output: [
          {
            value: "Error pushing files. Please try again",
            color: TERMINAL_COLORS.red,
          },
          ...(remote !== "origin"
          ...
          {
            value:
              "hint: In order to push to a remote repository, you MUST have accepted GitHub organisation invite.",
          },
          { value: pushHint },
        ],
      };
    ...
    if (branchName) {
      if (branchName.startsWith("-") || branchName.startsWith("--")) {
        return {
          ...
          output: [
            ...
            {
              value:
                "hint: Use 'git branch <branchName>' to create a new branch.",
            },
          ],
        };
      }
      ...
      try {
        ...
        return {
          ...
          output: [
            ...
            e.response.data?.message === "AlreadyExistsError"
              ? [
                  {
      ...
      return {
        ...
        output: [
          {
            value:
              "In actual git interface, this would result in listing out currently available branches.",
            color: TERMINAL_COLORS.yellow,
          },
          {
  ...
  async function processGitCheckout(args: string[]) {
    ...
    if (branchOrArg.startsWith("-") || branchOrArg.startsWith("--")) {
      if (branchOrArg === "-b") {
        ...
        try {
          ...
          console.log(
            "Error occurred while making a new branch for git checkout -b",
            e
          );
          return {
            input,
            output: [
              {
                value: "Error creating a branch to check out.",
                color: TERMINAL_COLORS.red,
              },
              ...(e instanceof AxiosError &&
              !!e.response &&
              ...
              e.response.data?.message === "AlreadyExistsError"
                ...
                : [
                    {
                      value:
                        "Unknown error occurred while creating a new branch. Please try again.",
                    },
                  ]),
            ],
          };
        ...
        return {
          ...
          output: [
            {
              value: "Unknown git checkout argument.",
              color: TERMINAL_COLORS.red,
            },
            {
              value:
                "Currently, the system only supports checking out to a new or an existing branch.",
            },
            {
              value:
                "hint: Use 'git checkout -b <branchName>' to create a new branch and check out to that branch.",
            },
            {
              value:
                "hint: Use 'git checkout <branchName>' to check out to an existing branch.",
            },
          ],
        };
      }
    ...
    if (branchName) {
      try {
        ...
        checkAndAdvanceScenarioSegment(
          isCreateBranch ? TaskType.BRANCH_CHECKOUT : TaskType.CHECKOUT
        );
        return {
          input,
          output: [
      ...
      return {
        ...
        output: [
          {
            value:
              "No branch specified to checkout. Please specify an existing branch to checkout.",
          },
          {
            value:
              "hint: Use 'git branch <branchName>' to first create a branch.",
          },
        ],
      };
    }
    ...
    if (branchName === "main") {
      try {
        ...
        return {
          input,
          output: [
            { value: `Successfully rebased and updated ${currentBranch}.` },
          ],
        };
      } catch (e) {
      ...
      return {
        ...
        output: [
          {
            value:
              "Currently, only rebase to main is supported. Please select main branch to rebase.",
          },
          {
      ...
      return {
        ...
        output: [
          {
            value:
              "No branch specified to rebase. Please specify an existing branch to rebase.",
          },
          {
            value:
              "hint: Perhaps you meant 'git rebase main' to rebase from the main branch?.",
          },
        ],
      };
    }
  ...
  async function processCommands(command: string): Promise<ConsolePrint> {
    const [commandType, gitCommand, ...args] = command.split(
      /\s+(?=(?:[^'"]*['"][^'"]*['"])*[^'"]*$)/
    );
    ...
    if (commandType !== "git") {
      return {
        input: command,
        output: [{ value: `"${command}" is not a git command.` }],
      };
    } else {
      switch (gitCommand) {
        case "status":
